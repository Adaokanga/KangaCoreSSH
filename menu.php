...
// --- LÓGICA DA API KANGA (MODIFICADA - VERSÃO COM PERSISTÊNCIA VIA CRONTAB) ---
function manageKangaAPI() {
    ...
    // Verificar se existe entrada no crontab para @reboot
    ...
    if($opt == '1') {
        ...
        // Adicionar ao crontab @reboot (se não existir)
        if(empty($cronCheck)) {
        ...
        if(!empty($cronCheck)) {
            $currentCron = shell_exec("crontab -l 2>/dev/null");
            // Remove linhas que contenham "kanga_api.php"
            $newCron = preg_replace('/^.*kanga_api\.php.*\n?/m', '', $currentCron);
            // Remove possíveis linhas em branco extras
            $newCron = trim($newCron);
            if(!empty($newCron)) {
                file_put_contents("/tmp/kanga_cron_tmp", $newCron);
                shell_exec("crontab /tmp/kanga_cron_tmp");
                unlink("/tmp/kanga_cron_tmp");
            } else {
                // Se ficou vazio, remove crontab inteiro
                shell_exec("crontab -r 2>/dev/null");
            }
            echo "\n[✔] Persistência removida do crontab.";
        } else {
