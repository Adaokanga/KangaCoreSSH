...
function drawHeader($subtitle = "HOME") {
    ...
    echo "{$blue}║{$reset}" . str_pad("\033[1m{$cyan}KANGA CORE{$version}{$reset}", 74, " ", STR_PAD_BOTH) . "{$blue}║{$reset}\n";
    echo "{$blue}╠" . str_repeat("═", 60) . "╣{$reset}\n";
...
// --- LÓGICA DA API KANGA (CORRIGIDA E TESTADA) ---
function manageKangaAPI() {
    system('clear');
    drawHeader("KANGA API REMOTA");
    
    // Verificar se a API está rodando no momento
    $pid = shell_exec("pgrep -f 'kanga_api.php'");
    $status = $pid ? "\033[1;32mATIVO (PID: ".trim($pid).")\033[0m" : "\033[1;31mINATIVO\033[0m";
    
    // Verificar se há entrada no crontab para @reboot
    ...
    if($opt == '1') {
        // --- LIGAR API ---
        if(!$pid) {
            // Inicia a API dentro de uma sessão screen para rodar em segundo plano
            shell_exec("screen -dmS kanga_api php -S 0.0.0.0:".API_PORT." ".API_PATH);
            echo "\n[✔] API iniciada com sucesso!";
        } else {
            echo "\n[!] API já está rodando.";
        }
        
        // Adicionar ao crontab @reboot se ainda não existir
        if(empty($cronCheck)) {
            // Comando que será executado após cada reboot do sistema
            $cronLine = "@reboot sleep 10 && screen -dmS kanga_api php -S 0.0.0.0:".API_PORT." ".API_PATH;
            $currentCron = shell_exec("crontab -l 2>/dev/null");
            
            if(empty($currentCron)) {
                // Cria um novo crontab
                file_put_contents("/tmp/kanga_cron_tmp", $cronLine . PHP_EOL);
                shell_exec("crontab /tmp/kanga_cron_tmp");
                unlink("/tmp/kanga_cron_tmp");
            } else {
                // Adiciona a nova linha ao final do crontab existente
        ...
        // Remove a linha do crontab que contém "kanga_api.php"
        if(!empty($cronCheck)) {
            $currentCron = shell_exec("crontab -l 2>/dev/null");
            $lines = explode("\n", $currentCron);
            $newLines = [];
            ...
            foreach ($lines as $line) {
                // Ignora qualquer linha que faça referência à API
                if (strpos($line, 'kanga_api.php') === false) {
                    $newLines[] = $line;
                }
            }
            ...
            $newCron = preg_replace("/\n{2,}/", "\n", $newCron);
            $newCron = trim($newCron);
            
            if (!empty($newCron)) {
                file_put_contents("/tmp/kanga_cron_tmp", $newCron . PHP_EOL);
                shell_exec("crontab /tmp/kanga_cron_tmp");
                unlink("/tmp/kanga_cron_tmp");
            } else {
                // Se não sobrar nenhuma linha, remove o crontab completamente
                shell_exec("crontab -r 2>/dev/null");
            }
            
            // Verifica se a remoção foi bem-sucedida
            $verify = shell_exec("crontab -l 2>/dev/null | grep 'kanga_api.php'");
            if (empty($verify)) {
                echo "\n[✔] Persistência removida do crontab com sucesso.";
            } else {
                echo "\n[!] Falha ao remover persistência. Verifique manualmente com 'crontab -e'.";
            }
        } else {
            echo "\n[!] Nenhuma entrada de persistência encontrada.";
        }
        sleep(2);
        
    } elseif($opt == '3') {
        // --- EXIBIR ENDPOINT DA API ---
        // Obtém o IP público do VPS
